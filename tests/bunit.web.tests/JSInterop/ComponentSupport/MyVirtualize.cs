...
{
	internal interface IMyVirtualizeJsCallbacks
	{
	...
	internal class MyVirtualizeJsInterop : IAsyncDisposable
	{
		private const string JsFunctionsPrefix = "Blazor._internal.Virtualize";

		private readonly IMyVirtualizeJsCallbacks _owner;

		private readonly IJSRuntime _jsRuntime;

		private DotNetObjectReference<MyVirtualizeJsInterop>? _selfReference;

		public MyVirtualizeJsInterop(IMyVirtualizeJsCallbacks owner, IJSRuntime jsRuntime)
		{
			_owner = owner;
	...
	public sealed class MyVirtualize<TItem> : ComponentBase, IMyVirtualizeJsCallbacks, IAsyncDisposable
	{
		private MyVirtualizeJsInterop? _jsInterop;
		...
		protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			if (firstRender)
			{
				_jsInterop = new MyVirtualizeJsInterop(this, JSRuntime);
		...
		void IMyVirtualizeJsCallbacks.OnBeforeSpacerVisible(float spacerSize, float spacerSeparation, float containerSize)
		{
		...
		void IMyVirtualizeJsCallbacks.OnAfterSpacerVisible(float spacerSize, float spacerSeparation, float containerSize)
		{
