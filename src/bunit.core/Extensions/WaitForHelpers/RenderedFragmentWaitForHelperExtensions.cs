...
namespace Bunit
{
	...
	public static class RenderedFragmentWaitForHelperExtensions
	{
		...
		public static async Task WaitForStateAsync(this IRenderedFragmentBase renderedFragment, Func<bool> statePredicate, TimeSpan? timeout = null)
		{
		...
		{
			WaitForStateAsync(renderedFragment, statePredicate, timeout).GetAwaiter().GetResult();
		}
		...
		{
			WaitForAssertionAsync(renderedFragment, assertion, timeout).GetAwaiter().GetResult();
		}
	}
}
