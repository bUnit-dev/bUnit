...
public static class MarkupMatchesAssertExtensions
{
	...
	/// <param name="userMessage">A custom user message to display in case the verification fails.</param>
	[AssertionMethod]
	public static void MarkupMatchesAsync([StringSyntax("Html")]this string actual, INode expected, string? userMessage = null)
	{
		if (actual is null)
			throw new ArgumentNullException(nameof(actual));
		if (expected is null)
		...
		var actualNodes = actual.ToNodeList(expected.GetHtmlParser());
		actualNodes.MarkupMatchesAsync(expected, userMessage);
	}
	...
	/// <param name="userMessage">A custom user message to display in case the verification fails.</param>
	[AssertionMethod]
	public static void MarkupMatchesAsync(this INodeList actual, INode expected, string? userMessage = null)
	{
		if (actual is null)
			throw new ArgumentNullException(nameof(actual));
		if (expected is null)
	...
	/// <param name="userMessage">A custom user message to display in case the verification fails.</param>
	[AssertionMethod]
	public static Task MarkupMatchesAsync(this IRenderedFragment actual, RenderFragment expected, string? userMessage = null)
	{
		if (actual is null)
			throw new ArgumentNullException(nameof(actual));
		if (expected is null)
		...
		return MarkupMatchesInternalAsync();

		async Task MarkupMatchesInternalAsync()
		{
			var testContext = actual.Services.GetRequiredService<TestContextBase>();
			var renderedFragment = (IRenderedFragment)await testContext.RenderInsideRenderTreeAsync(expected);
	...
	/// <param name="userMessage">A custom user message to display in case the verification fails.</param>
	[AssertionMethod]
	public static Task MarkupMatchesAsync(this INode actual, RenderFragment expected, string? userMessage = null)
	{
		if (actual is null)
			throw new ArgumentNullException(nameof(actual));
		if (expected is null)
		...
		async Task MarkupMatchesInternalAsync()
		{
			var context = actual.GetTestContext() ??
			              throw new InvalidOperationException("The TestContext has not been initialized.");
			var fragmentBase = await context.RenderInsideRenderTreeAsync(expected);
			var renderedFragment = fragmentBase as IRenderedFragment ?? await AdhocRenderRenderFragment(expected);
			MarkupMatches(actual, renderedFragment, userMessage);
		}
	}
	...
	/// <param name="userMessage">A custom user message to display in case the verification fails.</param>
	[AssertionMethod]
	public static Task MarkupMatchesAsync(this INodeList actual, RenderFragment expected, string? userMessage = null)
	{
		if (actual is null)
			throw new ArgumentNullException(nameof(actual));
		if (expected is null)
		...
		async Task MarkupMatchesInternalAsync()
		{
			...
			var renderedFragment = await context.RenderInsideRenderTreeAsync(expected) as IRenderedFragment
			                       ?? await AdhocRenderRenderFragment(expected);
			MarkupMatches(actual, renderedFragment, userMessage);
		}
	}
	...
	private static async Task<IRenderedFragment> AdhocRenderRenderFragment(this RenderFragment renderFragment)
	{
		...
		return (IRenderedFragment)await ctx.RenderInsideRenderTreeAsync(renderFragment);
	}
