...
The first example passes data to the `Items` parameter, and then it uses the `Template<TValue>` factory method in <xref:Bunit.ComponentParameterFactory>, wtakes the name of the `RenderFragment<TValue>` template parameter, and the `Func<TValue, string>` delegate as input.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTk2MDkwMCwyMTEzOTkxOTQsLTE5ND
c3MTQ3ODYsMjIxMzgyNDgxXX0=
-->